...
2026-09-25: INSTALL.md step 4 now sets ultimate owner trust with gpg --import-ownertrust. The dotfiles git config sets gpg.minTrustLevel=fully and merge.verifySignatures=true, so without owner trust both git verify-commit and git pull fail in dom0. The old text said the signature is verified regardless, which was wrong.
<!-- SECTION:NOTES:END -->
