...
Version:        0.0.1
Release:        1%{?dist}
Summary:        Discord terminal client in Qubes OS
Group:          qusal
...
if test "$1" = "1"; then
  ## Install
  qubesctl state.apply discord.create
  qubesctl --skip-dom0 --targets=tpl-discord state.apply discord.install
  qubesctl state.apply discord.install-binary
  qubesctl --skip-dom0 --targets=discord state.apply discord.configure
  qubesctl state.apply discord.appmenus
elif test "$1" = "2"; then
  ## Upgrade
  true
fi
...
%changelog
* Thu Sep 24 2026 Radek Janik <cyberwassp@gmail.com> - 432e5d3
- fix(discord): replace Signal copy wtih discordo
