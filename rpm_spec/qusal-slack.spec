...
Version:        0.0.1
Release:        1%{?dist}
Summary:        Slack terminal client in Qubes OS
Group:          qusal
...
if test "$1" = "1"; then
  ## Install
  qubesctl state.apply slack.create
  qubesctl --skip-dom0 --targets=tpl-slack state.apply slack.install
  qubesctl state.apply slack.install-binary
  qubesctl --skip-dom0 --targets=slack state.apply slack.configure
  qubesctl state.apply slack.appmenus
elif test "$1" = "2"; then
  ## Upgrade
  true
fi
